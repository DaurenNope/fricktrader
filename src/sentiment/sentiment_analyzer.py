...
class SentimentAnalyzer:
    ...
    def get_crypto_news_sentiment(self, pair: str = "BTC") -> float:
        ...
        try:
            ...
            positive_keywords = ['bullish', 'rally', 'surge', 'pump', 'moon', 'breakout', 'adoption']
            negative_keywords = ['bearish', 'crash', 'dump', 'fear', 'sell-off', 'correction', 'decline']
    ...
    def get_social_media_sentiment(self, pair: str = "BTC") -> Tuple[float, float]:
        ...
        try:
            ...
            logger.info(f"🐦 Twitter sentiment for {base_currency}: {twitter_sentiment:.3f}")
            logger.info(f"🔴 Reddit sentiment for {base_currency}: {reddit_sentiment:.3f}")
    ...
    def get_sentiment_score(self, pair: str = "BTC/USDT") -> float:
        ...
        try:
            ...
            logger.info(f"🎭 Composite sentiment for {pair}: {composite_sentiment:.3f} (FG:{fear_greed}, T:{twitter_sentiment:.2f}, N:{news_sentiment:.2f}, R:{reddit_sentiment:.2f})")
    ...
    def get_detailed_sentiment(self, pair: str = "BTC/USDT") -> SentimentData:
        ...
        # Fallback
        return SentimentData(composite_sentiment=composite_score, timestamp=datetime.now())