...
class ManualApprovalManager:
    ...
    def __init__(
        self, db_path: str = "signal_approvals.db", config: Optional[Dict] = None
    ):
        ...
        # Configuration
        self.signal_expiry_hours = self.config.get(
            "signal_expiry_hours", 4
        )  # 4 hours default
        self.auto_cleanup_interval = self.config.get(
        ...
        self.cleanup_thread = threading.Thread(
            target=self._cleanup_expired_signals, daemon=True
        )
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_status ON signal_approvals(status)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_timestamp ON signal_approvals(timestamp)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_expires_at ON signal_approvals(expires_at)"
                )
        ...
        try:
            with self.lock:
                ...
                approval = SignalApproval(
                    ...
                    + timedelta(hours=self.signal_expiry_hours),
                )
                ...
                if approval_id:
                    logger.info(
                        f"🔒 Signal intercepted for manual approval: {approval.pair} {approval.signal_type} (ID: {approval_id})"
                    )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """,
                    (
                        ...
                        approval.technical_score,
                        approval.onchain_score,
                        approval.sentiment_score,
                        approval.composite_score,
                        approval.price,
                        approval.volume,
                        json.dumps(approval.analysis_data or {}),
                        approval.reasoning,
                        approval.timestamp,
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    approval = SignalApproval(
                        ...
                        approval_timestamp=(
                            datetime.fromisoformat(row["approval_timestamp"])
    ...
    def approve_signal(
        self, signal_id: int, approved_by: str = "user", reason: str = ""
    ) -> bool:
        """
    ...
    def reject_signal(
        self, signal_id: int, rejected_by: str = "user", reason: str = ""
    ) -> bool:
        """
        Reject a signal
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(
                        f"❌ Signal {signal_id} rejected by {rejected_by}: {reason}"
                    )
                    return True
                else:
                    logger.warning(f"Signal {signal_id} not found or already processed")
    ...
    def batch_approve(
        self, signal_ids: List[int], approved_by: str = "user", reason: str = ""
    ) -> int:
        """
    ...
    def _execute_approved_trade(self, signal_id: int):
        ...
        try:
            ...
            logger.info(
                f"🚀 Executed approved trade: {approval.pair} {approval.signal_type}"
            )
    ...
    def _get_approval_by_id(self, signal_id: int) -> Optional[SignalApproval]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT * FROM signal_approvals WHERE id = ?", (signal_id,)
                )
                row = cursor.fetchone()
    ...
    def _send_approval_notification(self, approval: SignalApproval):
        ...
        try:
            ...
            logger.info(
                f"📧 Notification sent for signal approval: {approval.pair} {approval.signal_type}"
            )
