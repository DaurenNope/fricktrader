...
class ShortSellingTester:
    ...
    def test_short_signal_generation(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(50, len(df)):  # Skip first 50 for indicator calculation
            ...
            conditions = {
                ...
                'declining_trend': current['close'] < df.iloc[i-5]['close']  # 5-candle decline
            }
    ...
    def simulate_short_trade_performance(self, entry_price: float, market_data: pd.DataFrame) -> dict:
        """Simulate how a short trade would perform"""
        
        # Find entry point (when RSI > 75 and MACD bearish)
        entry_idx = None
        for i in range(50, len(market_data)):
            if (market_data.iloc[i]['rsi'] > 75 and 
                market_data.iloc[i]['macd'] < market_data.iloc[i]['macd_signal']):
                entry_idx = i
        ...
        for i in range(entry_idx + 1, len(market_data)):
            ...
            profit_pct = (entry_price - current_price) / entry_price * 100  # Short profit calculation
    ...
    def create_short_test_report(self) -> dict:
        ...
        report = {
            ...
            'short_selling_readiness': self._assess_short_readiness(signal_test, trade_simulation)
        }
