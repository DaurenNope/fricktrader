...
class AdvancedHedgingSystem:
    ...
    def analyze_portfolio_risk(self, active_trades: List[Dict]) -> Dict:
        ...
        stock_pairs = [t for t in active_trades if t.get('pair', '').split('/')[1] in ['USD', 'EUR']]
    ...
    def generate_hedge_recommendations(self, portfolio_risk: Dict, active_trades: List[Dict]) -> List[Dict]:
        ...
        if portfolio_risk['correlation_risk'] > self.correlation_threshold:
            ...
            recommendations.append({
                ...
                'priority': 'high' if portfolio_risk['correlation_risk'] > 0.9 else 'medium',
        ...
        if len(active_trades) >= 3:
            vol_hedge_size = portfolio_risk['total_exposure'] * 0.05  # 5% volatility hedge
        ...
        if portfolio_risk['concentration_risk'] > 0.4:
            ...
            largest_position = max(active_trades, key=lambda x: x.get('stake_amount', 0))
            hedge_size = largest_position.get('stake_amount', 0) * 0.2  # 20% of largest position
    ...
    def calculate_hedge_effectiveness(self, hedge_type: str, portfolio_risk: Dict) -> Dict:
        ...
        return effectiveness_models.get(hedge_type, {
            'risk_reduction': 0.3,
            'cost_estimate': 0.02,
    ...
    def optimize_hedge_portfolio(self, recommendations: List[Dict], budget_limit: float) -> List[Dict]:
        ...
        for hedge in recommendations:
            ...
            efficiency = (effectiveness['risk_reduction'] * priority_score) / max(cost, 1)
        ...
        sorted_hedges = sorted(recommendations, key=lambda x: x['efficiency_score'], reverse=True)
    ...
    def create_hedge_execution_plan(self, optimized_hedges: List[Dict]) -> Dict:
        ...
        execution_plan = {
            ...
            'expected_risk_reduction': sum([h['expected_risk_reduction'] for h in optimized_hedges]) / len(optimized_hedges) if optimized_hedges else 0,
            'execution_phases': []
        }
...
def main():
    ...
    recommendations = hedging_system.generate_hedge_recommendations(risk_analysis, mock_trades)
    print(f"\n🎯 Hedge Recommendations: {len(recommendations)}")
    
    for i, rec in enumerate(recommendations, 1):
        print(f"   {i}. {rec['type'].title()}: {rec['action']}")
        print(f"      Priority: {rec['priority'].upper()}, Size: ${rec['size_usd']:.0f}")
    ...
    for hedge in optimized:
        print(f"   • {hedge['type']}: ${hedge['estimated_cost']:.0f} cost, {hedge['expected_risk_reduction']:.1%} risk reduction")
    
    # Create execution plan
    execution_plan = hedging_system.create_hedge_execution_plan(optimized)
    print(f"\n📋 Execution Plan:")
    print(f"   Total Cost: ${execution_plan['total_cost']:.0f}")
    print(f"   Expected Risk Reduction: {execution_plan['expected_risk_reduction']:.1%}")
