...
class TradeDecisionCapturer:
    ...
    def __init__(self, db_path="trade_logic.db"):
        ...
        self.signal_weights = {"technical": 0.40, "onchain": 0.35, "sentiment": 0.25}
    ...
    def capture_decision(self, strategy_instance, dataframe, metadata):
        ...
        try:
            ...
            decision_data = {
                ...
                "sentiment_score": self._get_latest_value(
                    dataframe, "sentiment_score", 0.0
                ),
                "sentiment_reasoning": self._generate_sentiment_reasoning(dataframe),
                # Market Regime
                "market_regime": self._get_latest_value(
                    dataframe, "market_regime", "neutral"
                ),
                "regime_confidence": self._get_latest_value(
                    dataframe, "regime_confidence", 0.5
                ),
                # Composite Decision
                "composite_score": self._get_latest_value(
                    dataframe, "multi_signal_score", 0.0
                ),
        ...
        try:
            ...
            signals["rsi"] = {
                ...
                "signal": (
                    "oversold"
                    if rsi_value < 30
                    else "overbought" if rsi_value > 70 else "neutral"
                ),
            ...
            signals["moving_averages"] = {
                ...
                "weight": 0.25,
                "contribution": self._calculate_ma_contribution(
                    close_price, sma_20, sma_50
                ),
            }
        ...
        try:
            ...
            signals["whale_activity"] = {
                ...
                "signal": (
                    "bullish"
                    if whale_score > 0.6
                    else "bearish" if whale_score < 0.4 else "neutral"
                ),
                "weight": 0.40,
            ...
            # Exchange Flows
            exchange_flow = self._get_latest_value(
                dataframe, "exchange_flow_score", 0.0
            )
            signals["exchange_flows"] = {
                ...
                "signal": (
                    ...
                    else "bearish" if exchange_flow > 0.5 else "neutral"
                ),
                "weight": 0.30,
            ...
            signals["defi_activity"] = {
                ...
                "signal": (
                    "bullish"
                    if defi_score > 0.6
                    else "bearish" if defi_score < 0.4 else "neutral"
                ),
                "weight": 0.20,
            ...
            signals["network_activity"] = {
                ...
                "signal": (
                    ...
                    else "bearish" if network_score < 0.4 else "neutral"
                ),
                "weight": 0.10,
    ...
    def _capture_sentiment_signals(self, dataframe):
        ...
        try:
            ...
            # Twitter Sentiment
            twitter_sentiment = self._get_latest_value(
                dataframe, "twitter_sentiment", 0.0
            )
            signals["twitter_sentiment"] = {
                "value": twitter_sentiment,
                "signal": (
                    "bullish"
                    if twitter_sentiment > 0.6
                    else "bearish" if twitter_sentiment < 0.4 else "neutral"
                ),
                "weight": 0.40,
            ...
            # Reddit Sentiment
            reddit_sentiment = self._get_latest_value(
                dataframe, "reddit_sentiment", 0.0
            )
            signals["reddit_sentiment"] = {
                ...
                "signal": (
                    ...
                    else "bearish" if reddit_sentiment < 0.4 else "neutral"
                ),
                "weight": 0.30,
            ...
            signals["news_sentiment"] = {
                ...
                "signal": (
                    ...
                    else "bearish" if news_sentiment < 0.4 else "neutral"
                ),
                "weight": 0.20,
            ...
            signals["fear_greed_index"] = {
                ...
                "signal": (
                    "bullish"
                    if fear_greed < 30
                    else "bearish" if fear_greed > 70 else "neutral"
                ),
                "weight": 0.10,
    ...
    def _generate_technical_reasoning(self, dataframe):
        ...
        try:
            ...
            if rsi < 30:
                reasoning.append(
                    f"RSI at {rsi:.1f} indicates oversold conditions, suggesting potential upward reversal"
                )
            elif rsi > 70:
                reasoning.append(
                    f"RSI at {rsi:.1f} shows overbought conditions, indicating possible downward pressure"
                )
            else:
            ...
            if macd > macd_signal and len(dataframe) > 1:
                ...
                prev_signal = self._get_latest_value(
                    dataframe.iloc[:-1], "macdsignal", 0.0
                )
                if prev_macd <= prev_signal:
                    reasoning.append(
                        "MACD bullish crossover detected, momentum shifting positive"
                    )
            ...
            if close > sma_20 > sma_50:
                reasoning.append(
                    "Price above both 20 and 50 SMA, indicating strong uptrend"
                )
            elif close < sma_20 < sma_50:
                reasoning.append(
                    "Price below both 20 and 50 SMA, indicating strong downtrend"
                )
            ...
            if volume > volume_sma * 1.5:
                ...
                reasoning.append(
                    "Low volume suggests weak conviction in price movement"
                )
    ...
    def _generate_onchain_reasoning(self, dataframe):
        ...
        try:
            ...
            if whale_score > 0.7:
                reasoning.append(
                    "Strong whale accumulation detected, indicating institutional confidence"
                )
            elif whale_score < 0.3:
                reasoning.append(
                    "Whale distribution pattern observed, suggesting potential selling pressure"
                )
            ...
            return (
                reasoning
                if reasoning
                else ["On-chain analysis shows neutral conditions"]
            )
    ...
    def _generate_sentiment_reasoning(self, dataframe):
        ...
        try:
            ...
            if twitter_sentiment > 0.7:
                reasoning.append(
                    "Highly positive Twitter sentiment indicates strong community optimism"
                )
            elif twitter_sentiment < 0.3:
                reasoning.append(
                    "Negative Twitter sentiment suggests community pessimism"
                )

            fear_greed = self._get_latest_value(dataframe, "fear_greed_index", 50.0)
            if fear_greed < 25:
                reasoning.append(
                    "Extreme fear in market presents contrarian buying opportunity"
                )
            elif fear_greed > 75:
            ...
            return (
                reasoning
                if reasoning
                else ["Sentiment analysis shows neutral market mood"]
            )
    ...
    def _build_decision_tree(self, dataframe):
        ...
        try:
            tree = {"root": "Multi-Signal Analysis", "branches": []}
            ...
            tree["branches"].append(
                {
                    ...
                    * self.signal_weights["technical"],
                    "decision": (
                        ...
                        else "bearish" if technical_score < 0.4 else "neutral"
                    ),
                }
            )
            ...
            tree["branches"].append(
                {
                    ...
                    "decision": (
                        ...
                        else "bearish" if onchain_score < 0.4 else "neutral"
                    ),
                }
            )
            ...
            tree["branches"].append(
                {
                    ...
                    * self.signal_weights["sentiment"],
                    "decision": (
                        ...
                        else "bearish" if sentiment_score < 0.4 else "neutral"
                    ),
                }
            )
            ...
            # Final decision
            composite_score = sum(
                branch["weighted_score"] for branch in tree["branches"]
            )
            tree["final_score"] = composite_score
            tree["final_decision"] = (
                "BUY"
                if composite_score > 0.65
                else "SELL" if composite_score < 0.35 else "HOLD"
            )
    ...
    def _analyze_thresholds(self, dataframe):
        ...
        try:
            ...
            analysis["rsi"] = {
                ...
                "threshold_status": (
                    "triggered" if rsi < 30 or rsi > 70 else "not_triggered"
                ),
            }
            ...
            analysis["composite_score"] = {
                ...
                "threshold_status": (
                    "buy_triggered"
                    if composite > 0.65
                    else "sell_triggered" if composite < 0.35 else "neutral"
                ),
            }
        ...
        try:
            if "close" in dataframe.columns and len(dataframe) > 20:
                ...
                return {
                    ...
                    * np.sqrt(24),  # Annualized hourly volatility
                    "volatility_percentile": self._calculate_percentile(
                        returns.std(), returns.rolling(20).std()
                    ),
                    "volatility_trend": (
                        "increasing"
                        if len(returns) > 1 and returns.iloc[-1] > returns.iloc[-2]
                        else "decreasing"
                    ),
                }
            return {}
    ...
    def _determine_trend(self, dataframe):
        ...
        try:
            if "close" in dataframe.columns and len(dataframe) > 20:
                ...
                sma_50 = (
                    dataframe["close"].rolling(50).mean().iloc[-1]
                    if len(dataframe) > 50
                    else sma_20
                )
    ...
    def _find_support_resistance(self, dataframe):
        ...
        try:
            if (
                "high" in dataframe.columns
                and "low" in dataframe.columns
...
if __name__ == "__main__":
    ...
    if decision_id:
        ...
        print(
            f"📊 Decision data: {decision['pair']} - Score: {decision['composite_score']}"
        )
