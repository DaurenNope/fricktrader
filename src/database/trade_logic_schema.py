...
class TradeLogicDBManager:
    ...
    def init_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_trade_decisions_pair ON trade_decisions(pair)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_trade_decisions_composite_score ON trade_decisions(composite_score)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_signal_performance_decision_id ON signal_performance(decision_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_signal_performance_type ON signal_performance(signal_type)')
    ...
    def get_decision(self, decision_id):
        ...
        if row:
            ...
            # Parse JSON fields
            json_fields = [
                'technical_signals', 'technical_reasoning',
                'onchain_signals', 'onchain_reasoning',
                'sentiment_signals', 'sentiment_reasoning',
                'risk_assessment', 'decision_tree', 'threshold_analysis',
                'signal_weights', 'market_conditions', 'volatility_metrics',
                'correlation_data'
            ]
    ...
    def get_decision_by_trade_id(self, trade_id):
        ...
        try:
            cursor.execute('SELECT * FROM trade_decisions WHERE trade_id = ?', (trade_id,))
            row = cursor.fetchone()
            ...
            if row:
                ...
                # Parse JSON fields
                json_fields = ['technical_signals', 'technical_reasoning', 'onchain_signals', 
                              'onchain_reasoning', 'sentiment_signals', 'sentiment_reasoning',
                              'decision_tree', 'threshold_analysis', 'signal_weights',
                              'market_conditions', 'volatility_metrics', 'correlation_data',
                              'risk_assessment']
    ...
    def update_trade_outcome(self, decision_id, trade_id, profit_loss, duration, success):
