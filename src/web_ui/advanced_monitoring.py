...
class AdvancedTradingMonitor:
    ...
    def get_real_time_performance(self) -> Dict:
        ...
        try:
            # Get active trades
            active_response = requests.get(f"{self.freqtrade_api}/status", auth=self.auth, timeout=5)
            active_trades = active_response.json() if active_response.status_code == 200 else []
            
            # Get trade history
            history_response = requests.get(f"{self.freqtrade_api}/trades", auth=self.auth, timeout=5)
            ...
            return {
                ...
                'hedging_recommendations': self._get_hedging_recommendations(active_trades)
            }
            
        except Exception as e:
            return {'error': str(e), 'timestamp': datetime.now().isoformat()}
    
    def _calculate_performance_metrics(self, active_trades: List, trade_history: List) -> Dict:
    ...
    def _assess_portfolio_risk(self, active_trades: List) -> Dict:
        ...
        correlation_risk = len(crypto_pairs) / len(active_trades) if active_trades else 0
        
        # Calculate current drawdown
        total_profit = sum([t.get('profit_abs', 0) for t in active_trades])
        drawdown_pct = (total_profit / total_exposure * 100) if total_exposure > 0 else 0
        ...
        return {
            ...
            'largest_position': max([t.get('stake_amount', 0) for t in active_trades]) if active_trades else 0
        }
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if not recent_decisions:
                    return {'status': 'no_data', 'message': 'No recent trade decisions to analyze'}
                
                # Analyze signal accuracy
                high_confidence_trades = [d for d in recent_decisions if d[0] > 0.7]
                medium_confidence_trades = [d for d in recent_decisions if 0.5 < d[0] <= 0.7]
                
                high_conf_success = len([d for d in high_confidence_trades if d[1] > 0]) / len(high_confidence_trades) if high_confidence_trades else 0
                medium_conf_success = len([d for d in medium_confidence_trades if d[1] > 0]) / len(medium_confidence_trades) if medium_confidence_trades else 0
                ...
                return {
                    ...
                    'strategy_effectiveness': 'excellent' if high_conf_success > 0.8 else 'good' if high_conf_success > 0.6 else 'needs_improvement'
                }
    ...
    def monitor_entry_threshold_effectiveness(self) -> Dict:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                avg_score = sum([t[1] for t in strict_threshold_trades]) / len(strict_threshold_trades)
                
                return {
                    'trades_since_fix': len(strict_threshold_trades),
                    'success_rate': success_rate * 100,
                    'avg_signal_strength': avg_score,
                    'threshold_effectiveness': 'excellent' if success_rate > 0.8 else 'good' if success_rate > 0.6 else 'needs_adjustment',
                    'recommendation': 'Thresholds working well' if success_rate > 0.7 else 'Consider tightening thresholds further'
                }
