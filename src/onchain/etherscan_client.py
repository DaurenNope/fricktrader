...
class EtherscanClient:
    ...
    def get_whale_transactions(self, hours_back: int = 24) -> List[Dict]:
        ...
        for block_offset in range(0, min(blocks_back, 100), 10):  # Sample every 10th block
    ...
    def get_exchange_flows(self, hours_back: int = 24) -> Dict[str, Dict]:
        ...
        for exchange, wallets in self.exchange_wallets.items():
            ...
            for wallet in wallets[:2]:  # Limit to 2 wallets per exchange for rate limiting
    ...
    def _get_wallet_transactions(self, wallet_address: str, hours_back: int = 24) -> List[Dict]:
    ...
    def get_whale_activity_score(self, pair: str) -> float:
        ...
        try:
            ...
            if not whale_txs:
                ...
                frequency_score = min(len(whale_txs) / 50, 1.0)  # Normalize to 50 transactions
    ...
    def get_exchange_flow_score(self, pair: str) -> float:
        ...
        try:
            ...
            if not flows:
                ...
                total_net_flow = sum(exchange['net_flow'] for exchange in flows.values())
                total_volume = sum(exchange['inflow'] + exchange['outflow'] for exchange in flows.values())
